...
This keeps Docker on the default bridge network and maps the service only to host loopback. Publish it through nginx or an SSH tunnel for demos. The existing `new-api` container stays on port `3000`.
